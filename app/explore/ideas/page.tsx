...
import { Search, Sparkles, Filter, ArrowLeft, Zap, ChevronLeft, ChevronRight, X, SlidersHorizontal } from "lucide-react";
...
export default function ExploreIdeasPage() {
  ...
  const activeFiltersCount = selectedSectors.length + selectedStages.length + selectedSeeking.length;
  ...
  const FilterElements = () => (
    <>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        ...
        <button
          onClick={() => { setSelectedSectors([]); setSelectedStages([]); setSelectedSeeking([]); }}
      ...
      <div className="explore-cards-container" style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
            <Link href="/explore" style={{ color: "#7c3aed", fontSize: 12, fontWeight: 700, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 4 }}>
              <ArrowLeft size={13} /> Hub Explore
            </Link>
          </div>
          <h1 style={{ fontSize: 24, fontWeight: 800, color: "#0f172a", marginBottom: 4 }}>Ide Bisnis &amp; Startup</h1>
          <p style={{ fontSize: 13, color: "#64748b" }}>Eksplorasi peluang investasi tahap awal dari founder di seluruh Indonesia</p>
        </div>

        {/* Search + Filter Mobile Trigger Bar */}
        <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
          <div style={{ flex: "1 1 200px", position: "relative" }}>
            <Search size={15} color="#94a3b8" style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)" }} />
            <input
              type="text"
              placeholder="Cari nama ide, sektor, kota..."
              value={searchQuery}
          ...
          <button
            ...
            style={{
              padding: "10px 14px",
              background: activeFiltersCount > 0 ? "#7c3aed" : "#fff",
              color: activeFiltersCount > 0 ? "#fff" : "#374151",
              border: `1px solid ${activeFiltersCount > 0 ? "#7c3aed" : "#e2e8f0"}`,
              borderRadius: 10,
              fontSize: 13,
              fontWeight: 700,
              cursor: "pointer",
          ...
          >
            ...
            <option>Modal Tertinggi</option>
          </select>
        </div>

        {/* Listing count & active pills */}
        <div style={{ fontSize: 12, color: "#64748b", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
          <div>
            ...
            {boosted.length > 0 && <span style={{ marginLeft: 6, color: "#d97706", fontWeight: 700 }}>({boosted.length} Boosted)</span>}
          </div>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 5, background: "linear-gradient(135deg, #fef3c7, #fffbeb)", border: "1px solid #fcd34d", borderRadius: 999, padding: "3px 12px" }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 800, color: "#d97706", letterSpacing: "0.04em" }}>SPONSORED LISTINGS</span>
              </div>
              <div style={{ height: 1, flex: 1, background: "#e2e8f0" }} />
            </div>

            <div className="ideas-cards-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              {boosted.map((opp) => (
                <div key={opp.id} style={{ position: "relative" }}>
                  <div style={{ border: "2px solid #fcd34d", borderRadius: 14, overflow: "hidden", boxShadow: "0 4px 12px rgba(245, 158, 11, 0.08)" }}>
                    <OpportunityCard
                      opportunity={opp}
        ...
        {paged.length === 0 && (
          <div style={{ textAlign: "center", padding: "40px 20px", background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", color: "#64748b" }}>
            <Search size={36} color="#cbd5e1" style={{ margin: "0 auto 10px" }} />
            <div style={{ fontSize: 15, fontWeight: 700, color: "#0f172a" }}>Tidak ada ide bisnis yang cocok</div>
            <p style={{ fontSize: 12, color: "#94a3b8", marginTop: 4 }}>Coba ubah kata kunci pencarian atau reset filter di atas.</p>
          </div>
        )}
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, borderBottom: "1px solid #f1f5f9", paddingBottom: 12 }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: "#0f172a" }}>Filter Ide Bisnis</div>
              <button
                onClick={() => setMobileFilterOpen(false)}
            ...
            <div style={{ marginTop: 20, paddingTop: 16, borderTop: "1px solid #e2e8f0", display: "flex", gap: 10 }}>
              ...
              <button
                onClick={() => setMobileFilterOpen(false)}
                style={{ flex: 2, padding: "12px", background: "linear-gradient(135deg, #7c3aed, #2563eb)", color: "#fff", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: "pointer" }}
              >
